...
function LoginContent() {
    ...
    const next = searchParams.get('next') || (source === 'guestbook' ? '/guestbook' : '/me/admin');
    const supabase = createClient();

    const getErrorMessage = (err: string | null) => {
        switch (err) {
            case 'discord_required': return 'DISCORD_LOGIN_REQUIRED_FOR_ADMIN';
            case 'unauthorized_role': return 'ACCESS_DENIED: MISSING_REQUIRED_ROLE';
            case 'session_error': return 'AUTHENTICATION_SIGNAL_LOST';
            default: return null;
        }
    };
